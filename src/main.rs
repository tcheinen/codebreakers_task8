...
use sodiumoxide::crypto::secretbox::xsalsa20poly1305::{Key, Nonce, Tag};
...
fn brute_range(
    ...
    sender: Sender<(String, [u8; 78], Vec<u8>)>,
) {
    ...
    for ciphertext in ciphertexts {
        for i in ciphertext.start..ciphertext.end {
            ...
            let tag = Tag::from_slice(&ciphertext.data[28..44]).expect("lmao invalid tag");
            let mut decrypt = [0u8; 34];
            decrypt.copy_from_slice(&ciphertext.data[44..]);
            if let Ok(()) = sodiumoxide::crypto::secretbox::xsalsa20poly1305::open_detached(
                &mut decrypt,
                &tag,
                &nonce,
                &key,
...
fn main() {
    ...
    std::thread::spawn(move || {
        let mut log = OpenOptions::new()
            ...
            .open("decrypted.log")
            .unwrap();
        while let Ok((session_key, session, plaintext)) = r.recv() {
            writeln!(
                &mut log,
        ...
        for name in names {
            ...
            s.spawn(move |_| {
                for version in VERSIONS.split('\n').map(|x| x.trim_end()) {
                    brute_range(name, version, &CIPHERTEXTS, s2.clone());
                }
...
mod tests {
    ...
    fn try_decrypt(name: &str, version: &str, cipher: Ciphertext) -> (String, [u8; 78], Vec<u8>) {
        let (sender, r) = crossbeam_channel::unbounded();
        let res = unsafe { brute_range(name, version, &[cipher], sender.clone()) };
        r.recv().unwrap()
    }
    ...
    fn try_with_own() {
        ...
        assert_eq!(res.0, session_key_known);
        assert_eq!(res.2, control_message);
    }
